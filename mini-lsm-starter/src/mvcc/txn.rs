...
impl Transaction {
    ...
    pub fn commit(&self) -> Result<()> {
        ...
        // Serializable validation: this transaction conflicts if any transaction that committed
        // after our read snapshot (and before our own commit) wrote a key we read.
        ...
        if serializability_check {
            ...
            // Garbage collect committed txn data no future commit could ever need again.
...
impl TxnIterator {
    ...
    /// A local-storage tombstone (empty value) marks a key deleted within the transaction; skip
    /// over those so the iterator only ever surfaces live keys.
